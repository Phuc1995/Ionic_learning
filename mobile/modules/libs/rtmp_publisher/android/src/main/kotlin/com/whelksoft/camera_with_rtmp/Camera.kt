...
class Camera(
    ...
    private fun prepareCameraForRecordAndStream(fps: Int, bitrate: Int?) {
        ...
        var width = streamingProfile.videoFrameWidth - (streamingProfile.videoFrameWidth * 25 / 100)
        var height = streamingProfile.videoFrameHeight
    ...
    fun enableAudio(result: MethodChannel.Result) {
        if (rtmpCamera == null) {
            result.success(null)
            return
        }
        try {
            ...
            result.error("enableAudio", e.message, null)
            return
        }
        result.success(null)
    }
