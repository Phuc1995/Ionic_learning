...
class UploadConfirmPortraitWidget extends StatelessWidget {
  final title;
  final AccountVerifyStoreController accountVerifyStoreController;
  final String? image;

  UploadConfirmPortraitWidget(
      {Key? key, required this.accountVerifyStoreController, required this.title, this.image})
      : super(key: key);
  ...
  _buildState(file, context) async {
    if (file.path.toString() != '') {
      if (await _store.checkFileUpload(file.path.toString())) {
        accountVerifyStoreController.portraitController.value = file.path.toString();
      } else {
