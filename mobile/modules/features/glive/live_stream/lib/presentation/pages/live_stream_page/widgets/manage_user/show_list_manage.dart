...
class ShowIdolDetailSheet extends StatelessWidget {
  final Rx<LiveMessageDto> liveMessage;
  final String storageUrl;

  const ShowIdolDetailSheet({Key? key, required this.liveMessage, required this.storageUrl}) : super(key: key);
  ...
  void _handleAddCensor(BuildContext context, String fullName) async {
    ShowDialog().showMessage(context, fullName + "live_manage_censor_title".tr, "live_manage_confirm_button".tr, () async {
      final response = await AddViewerPermission().call(AddViewerPermissionParam(title: ViewerPermissionType.LIVE_ROOM_MANAGER, userUuid: liveMessage.value.id, isRemove: false));
  ...
  void _handleRemoveCensor(BuildContext context, String fullName) async {
    ShowDialog().showMessage(context, fullName + "live_manage_censor_remove_title".tr, "live_manage_confirm_button".tr, () async {
      final response = await AddViewerPermission().call(AddViewerPermissionParam(title: ViewerPermissionType.LIVE_ROOM_MANAGER, userUuid: liveMessage.value.id, isRemove: true));
      response.fold((l) => null, (r) {
  ...
  void _handleAddLimitUser(BuildContext context, String fullName) async {
    ShowDialog().showMessage(context, fullName + "live_manage_limit_title".tr, "live_manage_confirm_button".tr, () async {
      final response = await AddViewerPermission().call(AddViewerPermissionParam(title: ViewerPermissionType.LIVE_ROOM_LOCKED_CHAT, userUuid: liveMessage.value.id, isRemove: false));
      response.fold((l) => null, (r) {
  ...
  void _handleRemoveLimitUser(BuildContext context, String fullName) async {
    ShowDialog().showMessage(context, fullName + "live_manage_limit_remove_title".tr, "live_manage_confirm_button".tr, () async {
      final response = await AddViewerPermission().call(AddViewerPermissionParam(title: ViewerPermissionType.LIVE_ROOM_LOCKED_CHAT, userUuid: liveMessage.value.id, isRemove: true));
      response.fold((l) => null, (r) {
